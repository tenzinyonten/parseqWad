...
def main():
    ...
    test_set = SceneTextDataModule.TEST_TIB # TEST_BENCHMARK_SUB + SceneTextDataModule.TEST_BENCHMARK
    if args.new:
    ...
    for name, dataloader in datamodule.test_dataloaders(test_set).items():
        ...
        confidence = 0
        label_length = 0
        for imgs, labels, paths in tqdm(iter(dataloader), desc=f'{name:>{max_width}}'):
            res = model.test_step((imgs.to(model.device), labels, paths), -1)['output']
            total += res.num_samples
            correct += res.correct
            ned += res.ned
            confidence += res.confidence
            label_length += res.label_length
            df_rows.extend(res.df_rows)
    ...
    df.to_csv('testing/output_with_conf.csv', index=False)

    result_groups = {
        # 'Benchmark (Subset)': SceneTextDataModule.TEST_BENCHMARK_SUB,
        'Benchmark': SceneTextDataModule.TEST_TIB,
    }
    if args.new:
        result_groups.update({'New': SceneTextDataModule.TEST_TIB})
