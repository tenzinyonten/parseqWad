...
class BaseSystem(pl.LightningModule, ABC):
    ...
    def _eval_step(self, batch, validation: bool) -> Optional[STEP_OUTPUT]:
        ...
        df_rows = []
        for pred, prob, gt, path in zip(preds, probs, labels, paths):
            confidence += prob.prod().item()
            pred = self.charset_adapter(pred)
            # Follow ICDAR 2019 definition of N.E.D.
            ned += edit_distance(pred, gt) / max(len(pred), len(gt))
            if pred == gt:
                row = {'path': path, 'gt': gt, 'pred': pred, 'confidence': prob.prod().item(), 'confidence_string': str(prob), 'correct': True}
                correct += 1
            else:
                row = {'path': path, 'gt': gt, 'pred': pred, 'confidence': prob.prod().item(), 'confidence_string': str(prob), 'correct': False}
                
            total += 1
            label_length += len(pred)
            df_rows.append(row)
            
        return dict(output=BatchResult(total, correct, ned, confidence, label_length, loss, loss_numel, df_rows))
